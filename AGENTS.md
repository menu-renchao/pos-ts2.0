...
- Live-mode data and offline-harness data must stay isolated. Do not mix live-only samples, selectors, API setup, cleanup state, or one-run environment values into offline fixtures, offline harness state, or offline `test-data` constants.
- Offline data exists only for deterministic migration execution. Keep it disposable: after live migration succeeds, removing offline harness data, stub clients, and offline-only fixtures must not require live flow, selector, or client rewrites.
- Never use an offline seed, stub client, localStorage shim, or offline harness shortcut to fake live behavior. If live requires a real UI path, API, DB lookup, or external system and that dependency is missing, record the live gap and ask for the required business/environment input.
- Live selector and flow failures must be fixed to the real live DOM contract and source Python business flow. If the blocker is not a selector/flow issue but missing live data, API, DB, device, or external entrypoint, stop and confirm rather than inventing a substitute.
- Run live Playwright cases one at a time unless explicitly instructed otherwise, usually with `npx playwright test <spec> -g "<用例名>" --reporter=line --timeout=90000`. Live runs need browser/network permission.
...
- Always enter the app from `http://192.168.1.169:22080/kpos/front2/myhome.html`, then navigate through the UI flow to the target page.
