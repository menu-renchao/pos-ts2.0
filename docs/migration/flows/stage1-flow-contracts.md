...
7. For `CashDrawerFlow.openCashInChinesePage`, open POS home, switch language to Chinese, open Cash In/Out with password `11`, and read the page title text.
8. For `CashDrawerFlow.completeCashInAndOpenCashOutChinesePage`, complete Cash In with note `test`, close the source-equivalent cover, reopen Cash In/Out, read Cash Out page text, then complete Cash Out with the same note.
