...
65. POS-32002 path: configure automatic fixed taxed charge `auto_test_fixed` 10.00 for Dine In only, turn off combine-charge recalculation, create Dine In order A with `superman item1`, create To Go order B with `superman item2`, open recent B in Recall, combine it with order A, and read combined charge lines, subtotal, and tax.
66. POS-32004 path: configure manual percent charge `manu_test_perc` 10% as share-tip, turn off combine-charge recalculation, create Dine In order A with `superman item1` and manually apply the charge, create To Go order B with `superman item2`, read Report Fee Amount, combine A with B in Recall, then read Report Fee Amount again.
67. POS-32006 path: configure automatic percent share-tip charge `auto_test_perc` 10% with min_guest 6, turn off combine-charge recalculation, create Dine In order A with guest count 3 and `superman item1`, create Dine In order B with guest count 4 and `superman item2`, combine A with B, and read combined charge lines.
68. POS-32008 path: configure Dine In automatic fixed taxed charge `auto_test1` 10.00, Dine In manual percent share-tip charge `auto_test2` 10%, turn off combine-charge recalculation, create Dine In order A with `superman item1`, manually apply `auto_test2`, add custom fixed taxed charge 5.00, save, create Dine In order B with `superman item2`, manually apply `auto_test2`, add custom 10% non-taxed charge, save, read A/B charge snapshots in Recall, combine A with B, and read combined charge lines.
69. POS-32016 path: configure Delivery automatic percent share-tip charge `auto_test_perc` 10% with min_amount 20, turn on combine-charge recalculation, read Total Report Fee Amount, create Delivery order A with `superman item1` changed to 20.00, create Delivery order B with `superman item2` changed to 10.00, combine A with B, read combined subtotal and charge lines, then read Total Report Fee Amount again.
70. POS-32017 path uses `OrderEntryFlow.combineDeliveryAutoChargeIntoDineInWithoutRecalculatedCharge`: configure Delivery automatic percent share-tip charge `auto_test_perc` 10%, turn on combine-charge recalculation, create Delivery order A with `superman item1`, create Dine In order B with `superman item2`, open order A in Recall, combine B into it, and read combined charge lines.
71. POS-32006 recalculation path uses `OrderEntryFlow.combineOrdersMeetingServiceChargeGuestCountWithRecalculation`: configure automatic percent share-tip service charge `auto_test_perc` 10% with min_guest 6, turn on combine-charge recalculation, create Dine In order A with guest count 3 and order B with guest count 4, combine them, then read combined subtotal and charge lines.
72. POS-32023 path uses `OrderEntryFlow.combineManualShareTipChargeOrderWithRecalculation`: configure manual percent share-tip charge `auto_test_perc` 10%, turn on combine-charge recalculation, create Dine In order A with the manual charge, create To Go order B without a charge, read Report Fee Amount and A charge before combine, combine A with B, then read combined charge lines and Report Fee Amount again.
73. POS-32031 path uses `OrderEntryFlow.combineOrdersWithAutoManualAndCustomChargesWithRecalculation`: configure Dine In automatic fixed taxed charge `auto_test1` 10.00 and manual percent share-tip charge `auto_test2` 10%, turn on combine-charge recalculation, create two Dine In orders that each include the manual charge and a custom charge, read A/B charge snapshots, combine them, and read combined charge lines.
74. POS-32955 path uses `OrderEntryFlow.createOpenFoodAndReadChargeDialogItemNames`: enter Dine In, create Open Food item `OpenFoodName中文` priced 10.00, open the order charge dialog, read the charge dialog item-name list, and close the dialog.
75. POS-32934 path uses `OrderEntryFlow.switchOrderPageLanguageAndReadAddTexts`: force Default language, enter Dine In, order a dish and read Add text, switch to Chinese on the order page and order another dish, read `加1`, switch back to Default and read `Add` again.
76. POS-32963 path uses `OrderEntryFlow.applyThreeDecimalChargeAndReadAmount`: enter Dine In, order `groupSwitchDish`, edit the selected item price to 100.00, apply a custom 1.015% order charge, and read the displayed order charge amount.
77. POS-33063 path uses `OrderEntryFlow.applyAutoChargeAfterTaxAndReadAmounts`: open Admin, enable offline tax-after-charge-calculation behavior, configure Dine In automatic percentage charge `manu_test_fixed` 10%, enter Dine In, order `groupSwitchDish`, read subtotal, tax, and charge, then delete the charge and restore the setting.
78. POS-34555 path uses `OrderEntryFlow.partiallyPayCashWithChangeAndReadRecallStatus`: enter Dine In, order `groupSwitchDish`, change item price to 20.00, settle 10.00 by cash, then settle 20.00 by cash so the second payment includes change, open Recall recent order, and read status.
79. POS-35134 path uses `OrderEntryFlow.refundFirstItemAndReadPaymentRecord` with `taxExemptFirstItem=false`: enter To Go, order the first split-discount dish, read subtotal and tax as expected item refund, add the second dish, pay by cash, open Recall recent order, refund item 1, and read payment record 2.
80. POS-35135 path uses `OrderEntryFlow.refundFirstItemAndReadPaymentRecord` with `taxExemptFirstItem=true`: enter To Go, order the first split-discount dish, void its item tax, read subtotal and tax as expected item refund, add the second dish, pay by cash, open Recall recent order, refund item 1, and read payment record 2.
81. POS-35142 path uses `OrderEntryFlow.refundFirstDiscountedItemAndReadPaymentRecord`: enter To Go, order the first split-discount dish, apply 10% whole-order discount, read the discounted tax-included unpaid amount as expected refund, add the second dish, pay by cash, open Recall recent order, refund item 1, and read payment record 2.
82. POS-35150 path uses `OrderEntryFlow.refundFirstItemWithRequestedAmountAbovePaidAndReadPaymentRecord`: enter To Go, order one split-discount dish, read the tax-included paid amount, pay by cash, open Recall recent order, request refund-by-item with paid amount plus 10.00, and read payment record 2.
83. POS-35163 path uses `OrderEntryFlow.refundTwoItemsSequentiallyAndReadPaymentRecords`: enter To Go, order two split-discount dishes, record the first item tax-included amount and full order amount, pay by cash, refund the first available item, then refund the first available item again after the first refunded item is excluded.
84. POS-35166 path uses `OrderEntryFlow.refundCashPaymentWithChangeAndReadPaymentRecord`: enter To Go, order one split-discount dish, read tax-included amount due, cash-pay with amount due plus 2.00 to create change, open Recall, refund payment record 1, and read payment record 2.
...
- POS-32002 verifies combined order keeps exactly one `auto_test_fixed 10.00` charge and the displayed tax equals `(combined subtotal + charge) * 0.0825` when combine-charge recalculation is off.
- POS-32004 verifies combined order keeps `manu_test_perc` at the exact pre-combine amount and Report Fee Amount stays unchanged after combining.
- POS-32006 verifies no automatic service charge is added after combining two individually non-qualifying orders, even though combined guest count reaches the configured min_guest.
- POS-32008 verifies combined order displays exactly three charge lines, `auto_test1` is aggregated to `20.00`, `auto_test2` equals the sum of the two pre-combine manual percent charges, and combined total charge equals order A plus order B pre-combine charge totals.
- POS-32016 verifies combine-charge recalculation recomputes `auto_test_perc` from the combined Delivery subtotal, the combined charge total equals that recomputed charge, and Total Report Fee Amount increases by the recomputed share-tip charge.
- POS-32017 verifies a Delivery-only automatic charge is removed after combine-charge recalculation when the combined order context no longer satisfies the Delivery order-type condition.
- POS-32006 recalculation verifies combined Dine In guest count 7 satisfies min_guest 6, `auto_test_perc` equals combined subtotal * 10%, and combined charge total equals that value.
- POS-32023 verifies manual share-tip charge `auto_test_perc` is preserved through combine-charge recalculation and Total Report Fee Amount remains unchanged.
- POS-32031 verifies recomputation keeps exactly three charge lines, recalculates automatic `auto_test1` to one 10.00 charge, preserves and aggregates manual `auto_test2`, preserves custom charges, and combined total equals A/B pre-combine totals minus one old automatic fixed charge.
- POS-32955 verifies the custom Open Food name `OpenFoodName中文` appears in the order charge dialog item list.
- POS-32934 verifies the order-page Add action text is `Add` in Default language, `加1` after switching to Chinese, and `Add` again after switching back to Default.
- POS-32963 verifies 100.00 * 1.015% is rounded and displayed as 1.02.
- POS-33063 verifies the automatic charge amount equals `(subtotal + tax) * 10%` when tax-after-charge-calculation mode is enabled.
- POS-34555 verifies the recalled order status is `Paid` after partial cash payment followed by an over-tendered cash payment.
- POS-35134 verifies refund-by-item returns the first taxable item's subtotal plus tax and appends payment record 2 as the negative refund amount.
- POS-35135 verifies refund-by-item returns the first tax-exempt item's subtotal without tax and appends payment record 2 as the negative refund amount.
- POS-35142 verifies refund-by-item returns the first item's discounted subtotal plus tax calculated after discount, and appends payment record 2 as the negative refund amount.
- POS-35150 verifies refund-by-item caps an over-requested refund amount to the original paid item amount and appends payment record 2 as that negative capped amount.
- POS-35163 verifies two sequential refund-by-item actions create payment record 2 and 3 as the negative first-item amount and negative remaining-item amount, proving the second selectable list excludes the first refunded item.
- POS-35166 verifies refunding an over-tendered cash payment uses the actual amount due, not the tendered amount including change.
...
- POS-32002 also uses `AdminPage.setupAutoFixedCharge`, `AdminPage.setAutoChargeTaxed`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.setCombineRecalculateCharge`, `OrderDishesPage.saveOrder`, `RecallPage.combineOrder`, `RecallPage.readOrderChargeItems`, `RecallPage.readOrderSubtotal`, and `RecallPage.readOrderTaxText` to validate taxed auto-charge combine behavior.
- POS-32004 also uses `AdminPage.setupManualPercentChargeAsTip`, `AdminPage.setCombineRecalculateCharge`, `ReportPage.readFeeAmount`, `OrderDishesPage.applyPresetCharge`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate share-tip manual charge combine behavior.
- POS-32006 also uses `AdminPage.setupAutoPercentChargeAsTip`, `AdminPage.setAutoChargeMinGuest`, `AdminPage.setCombineRecalculateCharge`, `OrderDishesPage.setGuestCount`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate min-guest auto-charge behavior when recomputation is disabled.
- POS-32008 also uses `AdminPage.setupAutoFixedCharge`, `AdminPage.setAutoChargeTaxed`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.setupManualPercentChargeAsTip`, `AdminPage.setManualChargeOrderTypes`, `AdminPage.setCombineRecalculateCharge`, `OrderDishesPage.applyPresetCharge`, `OrderDishesPage.applyCustomFixedOrderCharge`, `OrderDishesPage.applyCustomPercentOrderCharge`, `RecallPage.openRecentOrder`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate multi-charge aggregation when recomputation is disabled.
- POS-32016 also uses `AdminPage.setupAutoPercentChargeAsTip`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.setChargeMinAmount`, `AdminPage.setCombineRecalculateCharge`, `DeliveryPage.createDeliveryOrder`, `OrderDishesPage.changeSelectedItemPrice`, `ReportPage.readFeeAmount`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, `RecallPage.readOrderSubtotal`, and `RecallPage.readOrderChargeItems` to validate automatic delivery charge recalculation and share-tip report impact.
- POS-32017 uses `AdminPage.setupAutoPercentChargeAsTip`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.setCombineRecalculateCharge`, `DeliveryPage.createDeliveryOrder`, `OrderDishesPage.saveOrder`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate automatic charge removal when order-type applicability no longer matches.
- POS-32006 recalculation uses `AdminPage.setupAutoPercentChargeAsTip`, `AdminPage.setAutoChargeMinGuest`, `AdminPage.setCombineRecalculateCharge`, `OrderDishesPage.setGuestCount`, `RecallPage.combineOrder`, `RecallPage.readOrderSubtotal`, and `RecallPage.readOrderChargeItems` to validate min_guest recomputation.
- POS-32023 uses `AdminPage.setupManualPercentChargeAsTip`, `AdminPage.setCombineRecalculateCharge`, `OrderDishesPage.applyPresetCharge`, `ReportPage.readFeeAmount`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate manual share-tip preservation.
- POS-32031 uses `AdminPage.setupAutoFixedCharge`, `AdminPage.setAutoChargeTaxed`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.setupManualPercentChargeAsTip`, `AdminPage.setManualChargeOrderTypes`, `OrderDishesPage.applyPresetCharge`, `OrderDishesPage.applyCustomFixedOrderCharge`, `OrderDishesPage.applyCustomPercentOrderCharge`, `RecallPage.openRecentOrder`, `RecallPage.openPreviousOrder`, `RecallPage.combineOrder`, and `RecallPage.readOrderChargeItems` to validate automatic/manual/custom recomputation.
- POS-32955 uses `OrderDishesPage.openFoodWithoutTax`, `OrderDishesPage.openChargeDialog`, `OrderDishesPage.readChargeDialogItemNames`, and `OrderDishesPage.confirmChargeDialog` to validate Open Food item-name visibility in the charge dialog.
- POS-32934 uses `PosHomePage.switchLanguage`, `OrderDishesPage.addMenuItem`, and `OrderDishesPage.readAddActionText` to validate order-page language switching.
- POS-32963 uses `OrderDishesPage.changeSelectedItemPrice`, `OrderDishesPage.applyCustomPercentOrderCharge`, and `OrderDishesPage.readChargePrice` to validate three-decimal charge rounding.
- POS-33063 uses `AdminPage.setOfflineTaxCalculationIncludeCharge`, `AdminPage.setupAutoPercentCharge`, `AdminPage.setAutoChargeOrderTypes`, `AdminPage.deleteChargeByName`, `OrderDishesPage.readSubtotal`, `OrderDishesPage.readTax`, and `OrderDishesPage.readChargePrice` to validate tax-after-charge calculation.
- POS-34555 uses `OrderDishesPage.clickSettle`, `OrderDishesPage.modifySettlementPaymentAmount`, `OrderDishesPage.settleCurrentAmountByCash`, `RecallPage.openRecentOrder`, and `RecallPage.readOrderStatus` to validate cash partial payment with change.
- POS-35134 uses `OrderDishesPage.readSubtotal`, `OrderDishesPage.readTax`, `OrderDishesPage.settleByCash`, `RecallPage.refundByItem`, and `RecallPage.readPaymentRecordAmount` to validate taxable refund-by-item.
- POS-35135 also uses `OrderDishesPage.voidSelectedItemTax` before `OrderDishesPage.readSubtotal` and `OrderDishesPage.readTax` to validate tax-exempt refund-by-item.
- POS-35142 also uses `OrderDishesPage.applyWholeOrderDiscountPercent` and `OrderDishesPage.readSettlementUnpaidAmount` before adding the second dish to validate percent-discount refund-by-item.
- POS-35150 uses `OrderDishesPage.readSettlementUnpaidAmount`, `RecallPage.refundByItem` with requested amount, and `RecallPage.readPaymentRecordAmount` to validate refund cap behavior.
- POS-35163 uses `RecallPage.refundByItem` twice against the current available item list and `RecallPage.readPaymentRecordAmount` for records 2 and 3 to validate repeated refund behavior.
- POS-35166 uses `OrderDishesPage.modifySettlementPaymentAmount`, `OrderDishesPage.settleCurrentAmountByCash`, `RecallPage.refundPaymentRecord`, and `RecallPage.readPaymentRecordAmount` to validate change-excluded refund-by-amount.
...
- POS-32002 stub behavior persists the combine-charge recalculation setting, filters auto charges by order type so Dine In A receives `auto_test_fixed` and To Go B does not, snapshots taxed charge metadata during combine, displays combined subtotal from merged item rows, and recalculates Recall tax from the displayed combined subtotal plus taxed charge amount.
- POS-32004 stub behavior creates a manual percent charge with `shareTip=true`, snapshots share-tip metadata during combine, keeps the pre-combine charge amount in combined order details, and counts combined share-tip charge snapshots in Report Fee Amount after source order A is merged.
- POS-32006 stub behavior applies min_guest against each order at save time, keeps the 3-guest and 4-guest source orders free of auto charge, and when combine-charge recalculation is off it merges only existing charge snapshots instead of evaluating the combined 7-guest order against the active auto charge.
- POS-32008 stub behavior allows a saved order to carry a primary automatic charge plus extra manual/custom charge snapshots, computes custom fixed and percent charges at application time, persists tax/share-tip metadata, and when combine-charge recalculation is off aggregates combined charge snapshots by label so duplicate `auto_test1`, duplicate `auto_test2`, and duplicate custom `Charge` entries display as three summed lines.
- POS-32016 stub behavior marks Delivery-created orders as order type `delivery`, reevaluates automatic charges after item price edits, persists combined orders after Recall combine, and when combine-charge recalculation is on discards source snapshots and recalculates active automatic charge definitions against the merged order subtotal and order context.
- POS-32017 stub behavior preserves the target order type during combine; with combine-charge recalculation on, a Delivery-only automatic charge is not reapplied to the combined Dine In context and the combined charge list is empty.
- POS-32006 recalculation stub behavior persists guest counts on saved orders and applies automatic min_guest charge rules against the merged guest count and subtotal.
- POS-32023 stub behavior distinguishes manual primary charges from automatic charges during combine-charge recalculation, preserving the manual share-tip charge and avoiding duplicate Report Fee changes.
- POS-32031 stub behavior preserves manual and custom charge snapshots during combine-charge recalculation while recalculating only the active automatic charge definition for the merged Dine In order.
- POS-32955 stub behavior renders active order item names inside the charge dialog item list.
- POS-32934 stub behavior rerenders the order menu and Add action text when the language switch is clicked from the order page.
- POS-32963 stub behavior calculates custom percentage charges from the current active subtotal and rounds to two decimal places.
- POS-33063 stub behavior stores the tax-after-charge-calculation flag, calculates percentage charge base as subtotal plus item tax when enabled, and restores the flag after validation.
- POS-34555 stub behavior records partial cash payments, keeps the order partially paid after the first payment, marks the order Paid after the over-tendered second cash payment, and persists the saved payment records.
- POS-35134 stub behavior computes refund-by-item as item price plus item tax/default item tax, marks the item refunded, and appends a negative cash payment record.
- POS-35135 stub behavior applies item-level tax exemption before refund, so refund-by-item uses the item price without tax and appends a negative cash payment record.
- POS-35142 stub behavior prorates whole-order discount to the refunded item, computes refund tax from the discounted item amount, marks the item refunded, and appends a negative payment record.
- POS-35150 stub behavior accepts a requested refund amount, then caps refund-by-item to the lesser of requested amount, item refund amount, and remaining paid amount.
- POS-35163 stub behavior treats refund-by-item index as an index into currently refundable items, marks refunded items as unavailable, and appends one negative payment record per refund.
- POS-35166 stub behavior stores over-tendered cash payments as the actual unpaid amount, excluding change from the payment record and default refund amount.
...
| POS-32002-live | Real ChargeAPI add/delete taxed auto charge, Admin combine-recalculate setting selector, charge order-type filtering, Dine In/To Go order creation, Recall record index selection, combine dialog, combined charge detail, subtotal, tax selectors, and order sync are only stub-verified | Run live smoke for POS-32002 and record selector/API/order-sync gaps before removing live gap |
| POS-32004-live | Real ChargeAPI add/delete manual percentage share-tip charge, Admin combine-recalculate setting selector, Report password/Total Report/Fee Amount selector, manual charge dialog, Recall record index selection, combine dialog, combined charge detail, and report sync are only stub-verified | Run live smoke for POS-32004 and record selector/API/report-sync gaps before removing live gap |
| POS-32006-live | Real ChargeAPI add/delete auto percentage share-tip min_guest charge, Admin combine-recalculate setting selector, guest-count entry, Dine In order creation, Recall record index selection, combine dialog, combined charge detail selector, and order sync are only stub-verified | Run live smoke for POS-32006 and record selector/API/order-sync gaps before removing live gap |
| POS-32008-live | Real Admin/ChargeAPI setup for automatic fixed taxed Dine In charge and manual percentage share-tip Dine In charge, custom fixed/percent order-charge dialog, Recall record index selection, combine dialog, combined charge aggregation, tax/share-tip metadata, and order sync are only stub-verified | Run live smoke for POS-32008 and record selector/API/order-sync gaps before removing live gap |
| POS-32016-live | Real ChargeAPI add/delete Delivery automatic percentage share-tip min_amount charge, Admin combine-recalculate selector, Delivery customer/address creation, item price edit selector, Recall record index selection, combine dialog, combined subtotal/charge selectors, Report Fee Amount sync, and order persistence are only stub-verified | Run live smoke for POS-32016 and record selector/API/report-sync/order-persistence gaps before removing live gap |
| POS-32017-live | Real Delivery-only automatic charge setup, Delivery/Dine In order creation, Recall record index selection, combine dialog, and combined charge selector are only stub-verified | Run live smoke for POS-32017 and record selector/order-type applicability gaps |
| POS-32006-recalc-live | Real service charge min_guest setup, guest-count entry, Recall combine, subtotal, and charge detail selectors are only stub-verified | Run live smoke for the recalculation-on POS-32006 path and record selector/order-sync gaps |
| POS-32023-live | Real manual percentage share-tip setup, Report Fee Amount sync, manual charge dialog, Recall combine, and charge detail selectors are only stub-verified | Run live smoke for POS-32023 and record report/selector gaps |
| POS-32031-live | Real auto/manual/custom charge setup, custom charge dialog, Recall combine, tax/share-tip metadata, and charge aggregation selectors are only stub-verified | Run live smoke for POS-32031 and record selector/API/order-sync gaps |
| POS-32955-live | Real Open Food dialog, mixed Chinese/English custom name entry, charge dialog item-name selector, and dialog close behavior are only stub-verified | Run live smoke for POS-32955 and record selector/input gaps |
| POS-32934-live | Real order-page language switch control, item option panel/Add button selector, and menu refresh behavior are only stub-verified | Run live smoke for POS-32934 and record selector/state gaps |
| POS-32963-live | Real selected item price edit, custom charge percent input accepting three decimals, rounding behavior, and charge amount selector are only stub-verified | Run live smoke for POS-32963 and record selector/rounding gaps |
| POS-33063-live | Real AdminSettingAPI tax calculation setting, auto service charge setup/delete, tax/charge selectors, and cleanup restore are only stub-verified | Run live smoke for POS-33063 and record API/selector/cleanup gaps |
| POS-34555-live | Real settlement preset amount button, cash tender/change behavior, payment completion navigation, and Recall status selector are only stub-verified | Run live smoke for POS-34555 and record payment/selector gaps |
| POS-35134-live | Real refund-by-item setting fixture, refund dialog, item selection, refund amount display, and payment record selector are only stub-verified | Run live smoke for POS-35134 and record refund selector/API gaps |
| POS-35135-live | Real item tax void selector plus refund-by-item dialog and payment record selector are only stub-verified | Run live smoke for POS-35135 and record item-tax/refund selector gaps |
| POS-35142-live | Real refund-by-item setting, tax-after-discount setting, whole-order percent discount dialog, discounted-tax calculation, refund dialog, and payment record selector are only stub-verified | Run live smoke for POS-35142 and record settings/refund selector gaps |
| POS-35150-live | Real refund-by-item requested amount input, paid-amount cap behavior, and payment record selector are only stub-verified | Run live smoke for POS-35150 and record refund amount selector gaps |
| POS-35163-live | Real refund-by-item selectable list after first refunded item, second refund path, and payment record 3 selector are only stub-verified | Run live smoke for POS-35163 and record refund-list selector gaps |
| POS-35166-live | Real cash over-tender/change handling, refund-by-amount default amount, and payment record selector are only stub-verified | Run live smoke for POS-35166 and record payment/refund selector gaps |
