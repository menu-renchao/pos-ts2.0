...
export class OrderEntryFlow {
  ...
  async copyOrderAfterChangingManualChargeTriggerToAutoWithMinAmountMismatch(
    homeUrl: string,
  ): Promise<AutoChargeCopyMinGuestResult> {
    if (!this.adminPage) {
      throw new Error('POS-27288 requires AdminPage');
    }
    ...
    await this.homePage.clickAdmin();
    await this.adminPage.setChargeTriggerMode('manu_test_fixed', 'auto');
    await this.adminPage.setChargeMinAmount('manu_test_fixed', 1000);
    await this.homePage.open(homeUrl);
    await this.homePage.clickRecall();
    await this.recallPage.openRecentOrder();
    await this.recallPage.copyCurrentOrder();
